...
## 현재 진행 (2026-05-19 기준, 최신 ?v=20260519v189-header-count-filtered-only)

### ★ 수강생 관리 디테일 다듬기 (v185~v189, 2026-05-19) ★
사용자가 PWA·관리자 화면을 실제로 써보며 발견한 미세 UX 이슈들 일괄 정리.

- **v185**: 학생 PWA 강의실 카드 라벨 "할 일" → "테스트". (사용자 요청, "테스트"가 학원 용어와 더 맞음)
- **v186**: 다음 3가지 묶음.
  - **엑셀 학생 import 컬럼 확장** (AdminPanel.jsx) — 템플릿에 학교급/학년/학교/수강 과목 4컬럼 추가. import 로직에서 '고1'/'중1'/'5학년' 또는 숫자만('1')+학교급 조합 모두 처리. 과목은 콤마/공백 split 후 [국어,영어,수학,과학]만 허용. import한 학생은 `is_approved=true` 자동 부여.
  - **수강생 관리에 과목 필터 추가** — 필터 행에 '과목' select 추가(SUBJECTS 4개). `students.subjects` 배열에 매칭. 학년 select는 학교급 미선택 시 disabled로 잠금(섞이지 않게).
  - **학생 PWA 이모지 제거** — 🎓('학원')/📧('이메일 인증 필요')/🗑('×')/👤('') 모두 텍스트로 교체. ✓/✕는 일반 UI 기호라 유지.
- **v187 → v189 (헤더 카운트 단순화 과정)**:
  - v187: "수강생 관리 (12 / 69명)" 형식 시도 (필터 결과 / 전체)
  - v188: 학년 dropdown placeholder "학년 전체" → "학년", 옵션에서 학교급 prefix 제거(value는 유지) — "고1/고2/고3" 표시 → "1학년/2학년/3학년"(value='고1' 등)
  - **v189 (최종)**: 헤더 카운트를 "필터 결과만" 표시 — "수강생 관리 (N명)" 한 값만. 필터 없으면 그게 곧 전체.
