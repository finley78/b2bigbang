...
## 현재 진행 (2026-05-11 기준, 최신 ?v=20260511v50-exam-pdf)
...
- v49~v50: 등록 파일을 라벨 칩(`시험지 1페이지 (.jpg) [열기]`)으로, 새 시험 발행 시 객관식/서술형 기본 0. 시험지 칸도 PDF 허용(`accept` 확장) — StudentPortal 응시 화면 시험지 뷰어가 `.pdf`면 `<iframe>` + "새 탭에서 열기" 링크로 표시. Edge Function이 분석할 때 PDF는 텍스트+페이지 둘 다 처리하므로 텍스트 PDF가 이미지보다 정확.
- 2026-05-10~11 추가 작업 (전부 push·배포 완료, ?v 순서 v25→v50):
