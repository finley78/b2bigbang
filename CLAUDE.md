...
## 현재 진행 (2026-05-12 기준, 최신 ?v=20260512v67-material-library)
...
- **v67 (1차 — TeacherPortal만)**:
  - 자료실 탭(`teacherView==='files'`)을 2섹션으로: ① **"시험·숙제용 분석 자료"**(새 도서관) ② "학생에게 보낼 자료"(기존 attachments 그대로). 도서관: `materials` state(=`exams` where kind='material', 전 선생님 공유), 검색·과목·학교급 필터, 카드(분석 완료/전 뱃지·문항 수·올린 선생님), [분석 보기][수정][재분석/Claude 분석][삭제]. `+ 새 자료 분석` → `materialFormOpen` 모달: 제목·과목·학교급·학년·설명/출처 + 시험지 파일 + 답안지·해설 파일 + 분석 범위(페이지·쓸 문항 번호)·정밀 체크박스 2개 → `[저장하고 Claude 문항 분석]`(submitMaterial(true)) / `[분석 없이 저장만]`. 함수: `loadMaterials/openMaterialForm/openMaterialFormForEdit/submitMaterial/reanalyzeMaterial/deleteMaterial`. material 파일 storage 경로 `materials/<teacher_id>/...` (attachments 버킷, public).
  - **시험·숙제 발행 폼에서 분석 관련 UI 제거**: 답안지·해설 파일칸·분석 범위 카드(페이지·정밀 체크박스)·`[저장 및 문항 분석]` 버튼 → 사라짐. 대신 상단에 **`[자료실에서 불러오기]` 버튼** → `materialPickerOpen` 모달(분석된 material만 목록) → 고르면 `loadMaterialIntoExam`: image_paths→existing_paths, answer_paths→answer_existing_paths, analysis·answer_key·question_count·text_question_count·choices_per_question·subject·analyze_student_model·`material_id` 채움(class/title/날짜는 선생님이). 불러왔으면 `examDraft.material_id` 세팅 + 초록 배너 + `[연결 해제]`. 발행 버튼은 이제 `[시험/숙제 발행]` = `submitExam(false)` (분석 호출 없음). `submitExam` row에 `analysis`·`material_id` 추가. **`deleteExam`: `material_id` 있으면 storage 파일 안 지움**(자료가 공유). 시험지 직접 올리기 input은 `!material_id`일 때만 노출. 객관식 정답 입력 그리드도 `answer_key` 키 기준(키 개수==question_count면 실제 번호, 아니면 1..N).
  - 모바일 뒤로가기 핸들러에 `materialFormOpen`/`materialPickerOpen` 추가.
- **남은 것 (다음 세션)**: ① **AdminPanel에도 같은 도서관 + 발행 폼 불러오기** 미러링(현재 AdminPanel은 옛 방식 그대로 — `adminSubmitLevelTest(true)` 등). ② StudentPortal/AdminPanel가 material 파일 잘 보여주는지 실제 확인(공유 경로). ③ material 수정 시 옛 storage 파일은 안 지움(고아 누적) — 필요하면 정리. ④ `runExamAnalysis`/`analyzingExamId`/`analysisOpenId`/`removeExamFilesTeacher('answer')` 등 TeacherPortal 죽은 코드 정리. ⑤ 시험별 문항 부분집합(`selected_questions`를 test에) — v67에선 material이 전체를 들고 test가 통째로 상속, test별 부분 선택은 미구현.
