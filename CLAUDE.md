...
## 현재 진행 (2026-05-11 기준, 최신 ?v=20260511v63-exact-grading)
...
- **v63 (중요 버그픽스)**: `analyze-student` Edge Function v2 — **객관식 점수·정답수·단원별 통계는 더 이상 Claude가 계산하지 않고 서버가 `answer_key`(없으면 `analysis.questions`의 mc answer)와 학생 답을 정규화 비교해 직접 계산**(자동 채점과 항상 일치). Claude는 `weak_topics/strengths/mistake_pattern/summary/recommendation/text_feedback`(서술형 평가)만 작성. `ai_analysis` = 서버 계산값(score/total/percentage/wrong_questions/by_topic) + Claude 작성값. 이전엔 Claude가 텍스트로 채점하다 점수가 자동채점과 어긋나는 버그(예: 자동 2/20인데 AI 7/20)가 있었음 — 근본 수정됨. `renderStudentAnalysis`·리포트 HTML에 `text_feedback`('서술형 평가') 표시 추가.
