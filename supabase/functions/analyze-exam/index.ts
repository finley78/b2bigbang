      ...
      questions: {
        ...
        items: {
          ...
          properties: {
            ...
            choice_explanations: {
              type: "array",
              description: "객관식 문항의 선지별 해설. 1번 선지부터 순서대로 모든 선지를 포함. 서술형이면 빈 배열 []. 정답 선지는 왜 정답인지, 오답 선지는 왜 오답이고 어떤 함정인지 설명한다.",
              items: {
                type: "object",
                properties: {
                  ...
                  role: {
                    type: "string",
                    enum: ["정답", "매력적인 함정", "지엽적", "포괄적", "반대 내용", "본문 무관", "기타 오답"],
                    description: "이 선지의 성격. '정답'=맞는 선지. '매력적인 함정'=얼핏 그럴듯하나 핵심을 빗나감(가장 많이 고르는 매력적 오답). '지엽적'=글의 일부·예시·세부사항만 가리켜 주제로는 좁음. '포괄적'=글의 범위보다 너무 넓거나 일반적임. '반대 내용'=글의 취지와 반대됨. '본문 무관'=글에 언급되지 않은 엉뚱한 내용. '기타 오답'=그 외. 서술형 채점에는 쓰지 않음."
                  },
                  why: { type: "string", description: "이 선지가 왜 정답/오답인지 한국어 1~2문장. 답안지·해설이 제공되면 그 근거를 활용." }
                },
...
const BASE_INSTRUCTION = [
  "위 자료는 한 학원의 시험지(또는 숙제지)와, 있다면 그 답안지·해설이다.",
  "받은 시험지의 모든 문항을 빠짐없이 분석해서 report_exam_analysis 도구로 보고하라.",
  "- 답안지·해설이 함께 제공되면 그것을 근거로 정답과 출제 의도, 선지 해설을 정확히 파악하라.",
  "- 답안지가 없거나 해당 문항의 해설이 없으면 시험지만 보고 최선을 다해 추정하되, 정답을 확신할 수 없으면 answer를 '확인 필요'로 둬라.",
  "- 객관식/서술형은 문제 형식으로 판단하라 (①②③④⑤ 같은 선지가 있으면 mc).",
  "- 객관식 문항은 choice_explanations에 모든 선지(1번부터 끝까지)를 넣어라. 정답 선지는 role='정답', 나머지 오답 선지는 가장 알맞은 함정 유형으로 분류하고 왜 오답인지 한두 문장으로 설명하라. 주제·요지·제목·빈칸 추론·글의 순서·문장 삽입·요약문 같은 유형은 특히 오답 선지가 '매력적인 함정'인지 '지엽적/포괄적'인지 '본문 무관/반대'인지 정확히 구분하라.",
  "- 서술형 문항은 choice_explanations를 빈 배열 []로 둬라.",
  "- 문제에 '두 개를 고르시오', '모두 고르시오', '적절한 것을 모두' 등 복수 정답을 요구하는 문항은 pick_count를 그 개수로 설정하고 answer에 정답들을 콤마로 연결해 적어라(예: '2,4'). 단일 정답 문항은 pick_count=1.",
  "- 한국 중·고등 교육과정 기준으로 단원/개념을 분류하라.",
