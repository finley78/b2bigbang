        ...
        React.createElement('div', { style:{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'16px' } },
          ...
          React.createElement('button', { onClick: async function() {
            try {
              ...
              var insertRes = await window.supabase.from('courses').insert({ subject_id: subjId, title:'새 강좌', teacher:'강사명', price:'0원', sort_order: state.courses.length+1 }).select('*, subjects(name,color)').single();
            ...
            React.createElement('div', { style:{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:editingCourse===c.id?'12px':0 } },
              ...
              React.createElement('div', { style:{ display:'flex', gap:'8px' } },
                editingCourse===c.id && React.createElement('button', { onClick: async ()=>{
                  ...
                  await window.supabase.from('courses').update({ title:c.name, teacher:c.teacher, price:c.price, badge:c.badge, description:c.description, intro:c.intro, target:c.target, curriculum:c.curriculum, teacher_desc:c.teacherDesc, youtube:c.youtube, subject_id:subj.data?.id }).eq('id', c.id);
                  alert('저장되었습니다!');
